function svmrun(usrcnmpfun, usrfile, svfile, tstart, tend, ...
...
hpl = repmat(NaN,size(usrdata,1),ntstep);
emt = repmat(NaN,size(usrdata,1),ntstep);
sig_acc =  repmat(NaN,size(usrdata,1),ntstep);
emt_new = repmat(NaN,size(usrdata,1),ntstep);
%ncrit = vpl;
...
%Saving outputs in folder inside current directory named based on variables
%of interest
% foldername = strcat('URA =', num2str(GEAS_URA),', PSAT = ',num2str(PSAT),', PCONST = ', num2str(PCONST),...
%     ', LLGRID = ', num2str(usrlatstep), ', TSTEP = ', num2str(tstep));
% mkdir('outputs',foldername);
% save(strcat(cd,'\outputs\', foldername, '\outputs.mat'), 'satdata', 'usrdata', ...
%      'sat_xyz','vpl','emt','sig_acc','hpl','emt_new','usrlatgrid','usrlongrid',...
%    'svfile');
